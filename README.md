...
(Deep Learning Indaba 2026), one question behind first place.**
...
Same model, same data: **~12% to 99% accuracy.** Measurement belongs in code;
...
one boundary that is legitimately ambiguous (weak coverage at the far end versus a
...
- **One model, two modes.** General-knowledge questions are answered by the *base*
  ...
  erode general capability (catastrophic forgetting is avoided by construction, not by mitigation).
...
- **Robustness by voting.** Every answer is a majority over 5 to 12 seeded generations,
